...
export class WaiteConfirmComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<WaiteConfirmComponent>,
    @Inject(MAT_DIALOG_DATA) public data, private transSer: TransportationService, private emailser: EmailService, private userSer: FamilyService) { }
  ...
  confirm() {
    ...
    this.list.filter(t => {
      if (t.completed)
        this.data.thisTrans.usersAndAddress.push({ user: t.id, address: t.address });
